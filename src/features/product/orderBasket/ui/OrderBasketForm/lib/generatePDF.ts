...
export const generatePDF = (basket: { productId: number; count: number; }[], products: any[], electricityCoast: number, course: number) => {
  ...
  const totalConsumption = selectedProducts.reduce(
    (total: number, product: IAsic) => total + (product.watt * product.count * HOURS_IN_DAY * DAYS_IN_MONTH) / WATT,
    0
  );

  const profitWithoutElectricity = selectedProducts.reduce(
    (total: number, product: IAsic) => total + product.profitDayAll * product.count * DAYS_IN_MONTH,
    0
  );

  const totalPrice = selectedProducts.reduce((total: number, product: IAsic) => total + product.price * product.count, 0);
